...
export async function fetchTopVolume(): Promise<TopStock[]> {
  await delay();
...
export async function fetchHistoricalPrices(symbol: string, days: number = 90): Promise<HistoricalPrice[]> {
  await delay();
  const stock = MOCK_STOCKS.find((s) => s.symbol === symbol.toUpperCase());
...
export async function searchStocks(query: string): Promise<TopStock[]> {
  await delay();
